...
# calculate panel zone dimensions
	set pzlat23  [expr 416/2.0];	# 中心线到梁上节点边缘的水平距离 (= 柱高的一半)
	set pzlat32  [expr 416/2.0+416/2.0];	# 中心线到梁下节点边缘的水平距离 (= 柱高的一半+裁切长度)
...
#定义预应力筋单元
set ABfrp 7.4e2;
element truss 60 4 30 $ABfrp $matID_T1; 	
element truss 61 7 21 $ABfrp $matID_T1; 	
...
# record drift histories
	# record drifts
	recorder Drift -file $dataDir/Drift-Story1.out -time -iNode 1000 -jNode 50 -dof 1 -perpDirn 2;
	
# record floor displacements	
	recorder Node -file $dataDir/Disp.out -time -node 50 -dof 1 disp;
...
# record tendons force 
	recorder Element -file $dataDir/PTstrandsForce1.out -time -ele 60  axialForce;	
	recorder Element -file $dataDir/PTstrandsForce2.out -time -ele 61  axialForce;	
